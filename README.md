...
## Deployment (local “undeployed” devnet)

This entry is deployed on the **local Midnight devnet** (`networkId: undeployed`),
which is the network the repo uses end-to-end (see [`compose.yml`](compose.yml)).
The deployment is verified twice: the test suite prints the contract address, and
the read-only indexer at `127.0.0.1:8088` returns the address as a
`ContractDeploy` action on the ledger.

```bash
docker compose up -d --wait
npm run test:local          # prints "TenderShield Contract Address:" from tests/deploy.test.ts
```

### Deployed contract address (query it yourself)

```graphql
query {
  contractAction(address: "f09a4b72156067f353c53a3cf06706dd1145f406f6f13d73421a9de6c85a6742") {
    __typename
...
| --- | --- | --- | --- |
| **Local (undeployed)** | `f09a4b72156067f353c53a3cf06706dd1145f406f6f13d73421a9de6c85a6742` | `f8109c34b901b4b2173f7181d1f2c328c001462f6a27a25e4e14a1b1d41940f1` | ✅ indexer returns `ContractDeploy` |
| Preview | *(optional)* | — | — |
| Preprod | *(optional)* | — | — |
...
> A Preview/Preprod run additionally requires a funded wallet for that network.
